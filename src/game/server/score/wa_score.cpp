...
void CWebappScore::OnUserRankMap(IResponse *pResponse, bool ConnError, void *pUserData)
{
	...
	if(!Error)
	{
		...
		if(pJsonData)
		{
			...
			{
				int Time = IRace::TimeFromSecondsStr(BestRun["time"]);
				...
				for(unsigned int i = 0; i < CpNum; i++)
					aCheckpointTimes[i] = IRace::TimeFromSecondsStr(CheckpointList[i]);
	...
	if(Own && MapRank)
	{
		...
		if(g_Config.m_SvShowBest && Run.m_Time > 0)
		{
			...
			if(g_Config.m_SvShowTimes)
				...
				pScore->GameServer()->SendPlayerTime(pUser->m_ClientID, Run.m_Time, pUser->m_ClientID);
		}
	}
