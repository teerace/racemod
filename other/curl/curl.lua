Curl = {
	...
	OptFind = function (name, required)
		local check = function(option, settings)
			...
			if ExecuteSilent("pkg-config") > 0 and ExecuteSilent("pkg-config libcurl") == 0 then
				option.value = true
		...
		local apply = function(option, settings)
			...
			elseif option.use_winlib > 0 then
				...
				if option.use_winlib == 32 then
					settings.link.libpath:Add(Curl.basepath .. "/windows/lib32")
				else
					settings.link.libpath:Add(Curl.basepath .. "/windows/lib64")
				end
